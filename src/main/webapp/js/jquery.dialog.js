...
$.dialog = {
	...
	confirm:function(set){
		...
		if(d.mask == true){addMask();}
		$("body").append('<div class="dialog" style="width:'+d.width+'px; height:'+d.height+'px; position:fixed; top:'+getTop(d.height)+'%; left:'+getLeft(d.width)+'%; z-index:1000;"></div>');
		$(".dialog").append('<div class="dialog_body"><div class="dialog_top">'+d.title+'</div><div class="dialog_main">'+d.content+'</div><div class="dialog_bottom"><span><input type="button" value="确定" class="ivu-btn ivu-btn-info" onclick="'+d.callback+'$.dialog.close();" />&nbsp;<input type="button" class="ivu-btn" value="取消" onclick="$.dialog.close();" /></span></div></div>');
	},
	/*加载页面*/
	load:function(set){
		d = defaults(set);
